...
@Field final String MAVEN_LOCAL_REPOSITORY = '.repository'
...
stage('Default build') {
	...
	node(NODE_PATTERN_BASE) {
		...
		withDefaultedMaven {
			...
			"""
			dir("$env.WORKSPACE/$MAVEN_LOCAL_REPOSITORY") {
...
void withDefaultedMaven(Map args, Closure body) {
	...
	args.putIfAbsent('mavenLocalRepo', MAVEN_LOCAL_REPOSITORY)
	withMaven(args, body)
}
