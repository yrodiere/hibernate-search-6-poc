...
public class OrmElasticsearchLibraryShowcaseIT {
	...
	public void search() {
		...
		withinSession( sessionFactory, session -> {
			LibraryDao dao = new LibraryDao( session );

			List<Library> libraries = dao.search( "org/hibernate/search/v6poc/integrationtest/showcase/library", 0, 10 );
			...
			libraries = dao.search( "org/hibernate/search/v6poc/integrationtest/showcase/library", 1, 2 );
			assertThat( libraries ).containsExactly(
					session.get( Library.class, SUBURBAN_1_ID ),
					session.get( Library.class, SUBURBAN_2_ID )
			);
	...
	public void search_single() {
		...
		withinSession( sessionFactory, session -> {
			DocumentDao dao = new DocumentDao( session );
	...
	public void searchAroundMe_spatial() {
		...
		DocumentDao dao = new DocumentDao( sessionFactory.createEntityManager() );
	...
	public void searchAroundMe_searchBridge() {
		// TODO bridges in search queries
		assumeTrue( "Bridges in search queries not implemented yet", false );
	...
	private void initData(Session session) {
		LibraryDao libraryDao = new LibraryDao( session );
