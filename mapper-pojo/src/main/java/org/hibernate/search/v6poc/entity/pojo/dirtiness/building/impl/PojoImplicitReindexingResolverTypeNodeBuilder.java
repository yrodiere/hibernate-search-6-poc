...
class PojoImplicitReindexingResolverTypeNodeBuilder<T>
	...
	public PojoImplicitReindexingResolverValueNodeBuilderDelegate<?> property(PropertyHandle propertyHandle) {
		return getOrCreatePropertyBuilder( propertyHandle )
				.valueWithoutExtractors();
	}

	@Override
	public Optional<PojoImplicitReindexingResolverValueNodeBuilderDelegate<?>> property(PropertyHandle propertyHandle,
			Class<? extends ContainerValueExtractor> extractorClass) {
		return property( propertyHandle, Collections.singletonList( extractorClass ) );
	}
	...
	public Optional<PojoImplicitReindexingResolverValueNodeBuilderDelegate<?>> property(PropertyHandle propertyHandle,
		...
		return getOrCreatePropertyBuilder( propertyHandle )
				.value( ContainerValueExtractorPath.explicitExtractors( extractorClasses ) );
	}
