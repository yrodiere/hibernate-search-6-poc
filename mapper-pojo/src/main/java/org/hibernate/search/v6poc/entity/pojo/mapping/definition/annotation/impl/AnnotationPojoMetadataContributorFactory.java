...
class AnnotationPojoMetadataContributorFactory {
	...
	AnnotationPojoMetadataContributorFactory(BeanProvider beanProvider, PojoRawTypeModel<?> typeModel) {
		this.beanProvider = beanProvider;
		this.typeModel = typeModel;
	}

	public <A extends Annotation> PojoPropertyMetadataContributor marker(A annotation) {
		return new AnnotationPojoPropertyMetadataContributor<A>( annotation ) {
			@Override
	...
	public PojoPropertyMetadataContributor associationInverseSide(AssociationInverseSide annotation,
			PojoPropertyModel<?> annotationHolder) {
		return new AnnotationPojoPropertyMetadataContributor<AssociationInverseSide>( annotation ) {
			@Override
			void doContributeModel(PojoAdditionalMetadataCollectorPropertyNode collector) {
				...
				if ( !inversePathOptional.isPresent() ) {
					throw log.missingInversePathInAssociationInverseSideMapping( typeModel, annotationHolder.getName() );
				}
	...
	public PojoPropertyMetadataContributor indexingDependency(IndexingDependency annotation,
			PojoPropertyModel<?> annotationHolder) {
		return new AnnotationPojoPropertyMetadataContributor<IndexingDependency>( annotation ) {
			@Override
			void doContributeModel(PojoAdditionalMetadataCollectorPropertyNode collector) {
				ContainerValueExtractorPath extractorPath = getExtractorPath(
				...
				if ( derivedFromAnnotations.length > 0 ) {
					...
					for ( ObjectPath objectPath : annotation.derivedFrom() ) {
						...
						if ( !pojoModelPathOptional.isPresent() ) {
							throw log.missingPathInIndexingDependencyDerivedFrom( typeModel, annotationHolder.getName() );
						}
	...
	public PojoPropertyMetadataContributor documentId(DocumentId annotation, PojoPropertyModel<?> annotationHolder) {
		return new AnnotationPojoPropertyMetadataContributor<DocumentId>( annotation ) {
			@Override
			void doContributeMapping(PojoMappingCollectorPropertyNode collector) {
				BridgeBuilder<? extends IdentifierBridge<?>> builder =
						createIdentifierBridgeBuilder( annotation, annotationHolder );
	...
	private <A extends Annotation> PojoTypeMetadataContributor routingKeyBridge(A annotation) {
		return new AnnotationPojoTypeMetadataContributor<A>( annotation ) {
			@Override
	...
	private <A extends Annotation> PojoTypeMetadataContributor typeBridge(A annotation) {
		return new AnnotationPojoTypeMetadataContributor<A>( annotation ) {
			@Override
			void doContributeMapping(PojoMappingCollectorTypeNode collector) {
	...
	private <A extends Annotation> PojoPropertyMetadataContributor propertyBridge(A annotation) {
		return new AnnotationPojoPropertyMetadataContributor<A>( annotation ) {
			@Override
			void doContributeMapping(PojoMappingCollectorPropertyNode collector) {
	...
	private PojoPropertyMetadataContributor field(Field annotation, PojoPropertyModel<?> annotationHolder) {
		return new AnnotationPojoPropertyMetadataContributor<Field>( annotation ) {
			@Override
			void doContributeMapping(PojoMappingCollectorPropertyNode collector) {
				...
				BridgeBuilder<? extends ValueBridge<?, ?>> builder = createValueBridgeBuilder( annotation, annotationHolder );
	...
	private PojoPropertyMetadataContributor indexedEmbedded(IndexedEmbedded annotation) {
		return new AnnotationPojoPropertyMetadataContributor<IndexedEmbedded>( annotation ) {
			@Override
			void doContributeMapping(PojoMappingCollectorPropertyNode collector) {
