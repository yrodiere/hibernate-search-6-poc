 ...
 */
class ChangesetPojoIndexedTypeWorkerTest<I, E, D extends DocumentElement> extends ChangesetPojoTypeWorker {

	private final EntityStuff<I, E> entityStuff;
	...
	ChangesetPojoIndexedTypeWorkerTest(PojoIndexedTypeManager<I, E, D> typeManager, PojoSessionContext sessionContext,
	...
	EntitySupplier<I, E> wrap(Object providedId, Object entity) {
		Supplier<E> entitySupplier = entityStuff.toEntitySupplier( sessionContext, entity );
		I identifier = entityStuff.getIdentifier( providedId, entitySupplier );
		return new EntitySupplier( identifier, entitySupplier ); // Or make identifier extraction lazy
	}

	EntitySupplier<I, E> wrap(Object entity, PostUpdateEvent event) {
		return wrap( typeManager.tryExtractIdFromEvent( event ), entity );
	}

	EntitySupplier<I, E> wrap(Object entity, AbstractCollectionEvent event) {
		return wrap( typeManager.tryExtractIdFromEvent( event ), entity );
	}

	@Override
	void add(Object providedId, Object entity) {
		EntitySupplier<I, E> wrapped = wrap( providedId, entity );
		getWork( wrapped ).add( wrapped );
	}

	@Override
	void update(Object providedId, Object entity) {
		EntitySupplier<I, E> wrapped = wrap( providedId, entity );
		getWork( wrapped ).update( wrapped );
	}

	@Override
	void update(Object entity, PostUpdateEvent event) {
		EntitySupplier<I, E> wrapped = wrap( entity, event );
		getWork( wrapped ).update( wrapped, event );
	}

	@Override
	void update(Object entity, AbstractCollectionEvent event) {
		EntitySupplier<I, E> wrapped = wrap( entity, event );
		getWork( wrapped ).update( wrapped, event );
	}
	...
	void delete(Object providedId, Object entity) {
		EntitySupplier<I, E> wrapped = wrap( providedId, entity );
		getWork( wrapped ).delete( entitySupplier );
	}

	void updateBecauseOfContained(Object entity) {
		EntitySupplier<I, E> wrapped = wrap( providedId, entity );
		if ( !workPlansPerId.containsKey( wrapped.getIdentifier() ) ) {
			getWork( wrapped ).updateBecauseOfContained( entitySupplier );
		}
	...
	private WorkPlanPerDocument getWork(EntitySupplier<I, E> wrapped) {
		I identifier = wrapped.getIdentifier();
	...
	private class WorkPlanPerDocument<S> {
