/*
 * Hibernate Search, full-text search for your domain model
 *
 * License: GNU Lesser General Public License (LGPL), version 2.1 or later
 * See the lgpl.txt file in the root directory or <http://www.gnu.org/licenses/lgpl-2.1.html>.
 */
package org.hibernate.search.v6poc.entity.pojo.mapping.impl;

import org.hibernate.search.v6poc.entity.pojo.mapping.spi.PojoSessionContext;

/**
 * @param <I> The type of per-type workers for indexed types.
 * @param <C> The type of per-type workers for contained types.
 */
public interface PojoWorkMode<I, C> {

	I createIndexedTypeWorker(PojoIndexedTypeManager<?, ?, ?> typeManager,
			PojoSessionContext sessionContext);

	C createContainedTypeWorker(PojoContainedTypeManager<?> typeManager,
			PojoSessionContext sessionContext);

}
