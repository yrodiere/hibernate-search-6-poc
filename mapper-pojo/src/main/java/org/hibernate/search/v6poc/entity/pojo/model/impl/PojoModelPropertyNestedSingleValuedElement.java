...
/**
 * The element obtained when getting property values on another element where extractors were not applied.
 *
 * @see PojoModelPropertyNestedMultiValuedElement
 */
class PojoModelPropertyNestedSingleValuedElement<T> extends AbstractPojoModelSingleValuedElement<T>
		implements PojoModelProperty, PojoPropertyNodeModelCollector {

	private final AbstractPojoModelSingleValuedElement<?> parent;

	private final PojoPropertyModel<?> propertyModel;

	private final Map<Class<?>, List<?>> markers = new HashMap<>();

	PojoModelPropertyNestedSingleValuedElement(AbstractPojoModelSingleValuedElement<?> parent,
	...
	<U> PojoModelElementAccessor<U> doCreateAccessor(PojoTypeModel<U> typeModel) {
		return new PojoModelPropertyNestedSingleValuedElementAccessor<>(
