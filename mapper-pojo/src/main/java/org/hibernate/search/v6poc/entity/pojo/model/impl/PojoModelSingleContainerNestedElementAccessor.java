...
class PojoModelSingleContainerNestedElementAccessor<C, T> implements PojoModelMultiValuedElementAccessor<T> {

	private final PojoModelElementAccessor<C> parent;
	private final ContainerValueExtractor<? super C, T> extractor;

	PojoModelSingleContainerNestedElementAccessor(PojoModelElementAccessor<C> parent,
	...
	public Stream<T> read(PojoElement bridgedElement) {
		C parentValue = parent.read( bridgedElement );
		return extractor.extract( parentValue );
	}
