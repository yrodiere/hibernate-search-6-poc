 ...
 * }
 * </code></pre>
 * ... creating a context for the type of property {@code propertyOfB}, within the "declaring" context of type {@code A},
 * will result in a context able to determine that the type argument
 * Note that "type resolution" here only covers the resolution of a type parameter to the corresponding argument,
 * i.e. "shallow" resolution.
 * This class will not build new {@link ParameterizedType} instances to perform deep resolution.
 * For instance, given the type {@code ArrayList<? extends Map<String, Integer>>}
 * When asked
 */
